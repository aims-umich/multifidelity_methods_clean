...
class MFNN_Delta:
    def __init__(self, x_dim, out_dim=1, hidden=(128,128,128), lr=0.0005, epochs=500, wd=0.0, verbose=False):
        self.out_dim = out_dim
        self.device  = get_device()
        self.low     = MLP(in_dim=x_dim,           hidden=hidden, out_dim=out_dim)
        self.delta   = MLP(in_dim=x_dim + out_dim, hidden=hidden, out_dim=out_dim)
